...
public class PsychicWallRender extends EntityRenderer<PsychicWallEntity> {

    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "textures/psychic_wall_border.png");

    public PsychicWallRender(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn);
    }

    @Override
    public void render(@NotNull PsychicWallEntity entity, float yaw, float partialTicks, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int light) {
        ...
        renderWall(poseStack, f, buffer, buffer.getBuffer(ModRenderTypes.PSYCHIC_WALL));
        renderWall(poseStack, 1, buffer, buffer.getBuffer(ModRenderTypes.PSYCHIC_WALL_BORDER));
        poseStack.popPose();
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(@NotNull PsychicWallEntity entity) {
        return TEXTURE;
    }
