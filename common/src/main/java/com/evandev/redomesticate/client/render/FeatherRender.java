...
public class FeatherRender extends EntityRenderer<FeatherEntity> {

    private ItemStack feather = new ItemStack(Items.FEATHER);

    public FeatherRender(EntityRendererProvider.Context renderManagerIn) {
    ...
    public void render(FeatherEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int light) {
        ...
        if (player != null) {
            ...
            if (!itemstack.is(ModItems.FEATHER_ON_A_STICK.get())) {
                i = -i;
            }
