...
import com.evandev.redomesticate.registry.ModRenderTypes;
...
public class LayerPetOverlays extends RenderLayer {
    ...
    private static final ItemStack MAGNET = new ItemStack(ModItems.MAGNET.get());
    ...
    private static final ResourceLocation AURA_TEXTURE = ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "textures/healing_aura.png");
    ...
    @Override
    public void render(@NotNull PoseStack matrixStackIn, @NotNull MultiBufferSource bufferIn, int packedLightIn, @NotNull Entity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (TameableUtils.isTamed(entity)) {
            ...
            if (TameableUtils.hasEnchant(living, ModEnchantments.IMMUNITY_FRAME) && TameableUtils.getImmuneTime((LivingEntity) entity) > 0) {
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(ModRenderTypes.IFRAME_GLINT);
                float alpha = 0.5F;
                matrixStackIn.pushPose();
                this.getParentModel().renderToBuffer(matrixStackIn, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0));
            ...
            if (TameableUtils.hasEnchant(living, ModEnchantments.VOID_CLOUD) && !living.isInWaterOrBubble() && !living.onGround() && TameableUtils.getFallDistance(living) >= 3.0F) {
                ...
                for (int i = 0; i < CLOUD_COUNT; i++) {
                    ...
                    renderVoidCloudCube(entity, matrixStackIn, bufferIn.getBuffer(ModRenderTypes.VOID_CLOUD));
                    matrixStackIn.popPose();
            ...
            if (shadowHandCount > 0) {
                ...
                for (int i = 0; i < shadowHandCount; i++) {
                    ...
                    VertexConsumer vertexconsumer = bufferIn.getBuffer(ModRenderTypes.SHADOW_HAND_ENTITY);
            ...
            if (TameableUtils.hasEnchant(living, ModEnchantments.BLAZING_PROTECTION)) {
                ...
                float f1 = realAge * 7;
                float seperation = 360F / (TameableUtils.getEnchantLevel(living, ModEnchantments.BLAZING_PROTECTION) * 2F);
                VertexConsumer vertexconsumer = bufferIn.getBuffer(Services.PLATFORM.getUnlitTranslucent(BLAZE_TEXTURE));
                matrixStackIn.pushPose();
                matrixStackIn.mulPose(Axis.YN.rotationDegrees(f));
                for (int i = 0; i < bars; i++) {
                    ...
                    BLAZING_BAR_MODEL.renderToBuffer(matrixStackIn, vertexconsumer, 240, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), 1);
                    matrixStackIn.popPose();
                }
                matrixStackIn.popPose();
            }
            if (TameableUtils.hasEnchant(living, ModEnchantments.HEALING_AURA)) {
                ...
                if (t > 0) {
                    ...
                    VertexConsumer vertexconsumer = bufferIn.getBuffer(ModRenderTypes.HEALING_AURA);
                    PoseStack.Pose posestack$pose = matrixStackIn.last();
                    Matrix4f matrix4f = posestack$pose.pose();
                    Matrix3f matrix3f = posestack$pose.normal();
                    vertex(vertexconsumer, matrix4f, posestack$pose, 240, 0.0F, 0, 0, 1, 1);
                    vertex(vertexconsumer, matrix4f, posestack$pose, 240, 1.0F, 0, 1, 1, 1);
                    vertex(vertexconsumer, matrix4f, posestack$pose, 240, 1.0F, 1, 1, 0, 1);
                    vertex(vertexconsumer, matrix4f, posestack$pose, 240, 0.0F, 1, 0, 0, 1);
                    matrixStackIn.popPose();
                }
            }
        }
