...
public class GiantBubbleEntity extends Entity {
    ...
    public void tick() {
        ...
        if (getPopsIn() <= 0) {
            ...
            this.level().addParticle(ModParticles.SIMPLE_BUBBLE.get(), this.getRandomX(1.4F), this.getRandomY(), this.getRandomZ(1.4F), (random.nextFloat() - 0.5F) * 0.3F, -0.1F, (random.nextFloat() - 0.5F) * 0.3F);
        }
    }
    ...
    private void pop() {
        this.playSound(ModSounds.GIANT_BUBBLE_POP.get(), 1.0F, 1.5F);
        if (!level().isClientSide()) {
            ((ServerLevel) this.level()).sendParticles(ModParticles.GIANT_POP.get(), this.getX(), this.getY() + this.getBbHeight() * 0.5F, this.getZ(), 1, 0, 0, 0, 0);
        }
