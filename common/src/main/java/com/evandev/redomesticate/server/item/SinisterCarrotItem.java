...
public class SinisterCarrotItem extends Item {
    ...
    public @NotNull InteractionResult interactLivingEntity(@NotNull ItemStack stack, @NotNull Player player, LivingEntity entity, @NotNull InteractionHand hand) {
        if (entity.getType() == EntityType.RABBIT && TameableUtils.isTamed(entity) && TameableUtils.isPetOf(player, entity) && Services.PLATFORM.canLivingConvert(entity, EntityType.RABBIT)) {
        ...
        if (entity.getType() == EntityType.ZOMBIE_HORSE && Services.PLATFORM.canLivingConvert(entity, EntityType.SKELETON_HORSE)) {
            player.swing(hand);
